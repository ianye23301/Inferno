...
@app.get("/jobs/{job_name}/all_metrics")
def get_all_metrics(job_name: str, sort_by: str = "total_output_tokens", order: str = "desc"):
    """
    Get all completed runs with their metrics, sorted by a specified field.
    
    Args:
        sort_by: Field to sort by (total_output_tokens, throughput_tok_s, accuracy, ttft_s)
        order: 'asc' or 'desc'
    """
    results = []
    ...
    if not results:
        raise HTTPException(404, f"No completed runs found for job '{job_name}'")
    
    # Sort results
    reverse = (order == "desc")
    try:
        results.sort(key=lambda x: x["metrics"].get(sort_by) or 0, reverse=reverse)
